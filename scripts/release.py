...
PACKAGE_INIT_PATH = ROOT / "src" / "genoprobe" / "__init__.py"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Prepare a GenoProbe release by cutting the CHANGELOG entry from "
            "Unreleased, updating version metadata, and running release checks."
        )
    )
    parser.add_argument(
        "version",
        help="Release version to write into pyproject.toml, src/genoprobe/__init__.py, and CHANGELOG.md.",
    )
    parser.add_argument(
        "--date",
        dest="release_date",
...
def update_package_version(content: str, version: str) -> str:
    ...
    if count != 1:
        raise ReleaseError("Could not find a unique __version__ line in src/genoprobe/__init__.py.")
    return updated
